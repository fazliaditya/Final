...
perkebunan_path = "Data_Login/peruntukan perkebunan.json"
if os.path.exists(perkebunan_path):
    with open(perkebunan_path, "r", encoding="utf-8") as f:
        perkebunan_geojson = json.load(f)

    def perkebunan_style(feature):
        return {
            'fillColor': '#A0522D',  # Coklat
            'color': 'black',
            'weight': 1,
            'fillOpacity': 0.6
        }
    ...
    m.add_geojson(
        ...
        style_function=perkebunan_style,
        show=True
    )
else:
    st.warning(f"File '{perkebunan_path}' tidak ditemukan.")
